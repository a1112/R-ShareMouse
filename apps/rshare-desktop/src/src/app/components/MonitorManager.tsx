...
interface MonitorManagerProps {
  ...
  showFooter?: boolean;
  onSaveLayout?: (monitors: MonitorData[]) => void;
  busy?: boolean;
}
...
export default function MonitorManager({
  ...
  const [edgeConnectMode, setEdgeConnectMode] = useState<{ monitorId: string; edge: "left" | "right" | "top" | "bottom" } | null>(null);
  ...
  /* ---- Handle edge click for connection mode ---- */
  const handleEdgeClick = useCallback((monitorId: string, edge: "left" | "right" | "top" | "bottom") => {
    if (!edgeConnectMode) {
      // Start edge connection mode
      setEdgeConnectMode({ monitorId, edge });
    } else {
      // Complete connection
      const { monitorId: fromId, edge: fromEdge } = edgeConnectMode;

      if (fromId === monitorId) {
        // Clicked same monitor, cancel
        setEdgeConnectMode(null);
        return;
      }

      // Calculate snap positions to connect the edges
      const fromMon = monitors.find((m) => m.id === fromId);
      const toMon = monitors.find((m) => m.id === monitorId);
      ...
      // Calculate position to snap edges together
      const EDGE_GAP = 0;

      switch (fromEdge) {
        case "right":
          newX = fromMon.x + fromMon.w + EDGE_GAP;
          // Align vertical center by default
          newY = fromMon.y + (fromMon.h - toMon.h) / 2;
          break;
        case "left":
          newX = fromMon.x - toMon.w - EDGE_GAP;
          newY = fromMon.y + (fromMon.h - toMon.h) / 2;
          break;
        case "bottom":
          newY = fromMon.y + fromMon.h + EDGE_GAP;
          // Align horizontal center by default
          newX = fromMon.x + (fromMon.w - toMon.w) / 2;
          break;
        case "top":
          newY = fromMon.y - toMon.h - EDGE_GAP;
          newX = fromMon.x + (fromMon.w - toMon.w) / 2;
          break;
      }

      setMonitors((prev) =>
        prev.map((m) => (m.id === monitorId ? { ...m, x: newX, y: newY } : m))
      );

      setEdgeConnectMode(null);
    }
  }, [edgeConnectMode, monitors]);

  const cancelEdgeConnect = useCallback(() => {
    setEdgeConnectMode(null);
  }, []);
      ...
      <div className="h-[36px] flex items-center px-3 gap-2.5 shrink-0" style={{ borderBottom: `1px solid ${t.border}`, background: t.toolbarBg }}>
        ...
        {onSaveLayout && (
          <button
            ...
            onMouseEnter={(e) => (e.currentTarget.style.backgroundColor = t.btnHover)}
            onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = t.btnBg)}
            onClick={() => onSaveLayout(monitors)}
            disabled={busy}
            title="保存布局到守护进程"
          >
            <Save size={11} /> {busy ? "保存中..." : "保存布局"}
          </button>
        )}
        ...
        {edgeConnectMode ? (
          <span className="text-[10px] px-2 py-1 rounded" style={{ color: "#fff", background: "#3b82f6" }}>
            连接模式：选择另一台显示器的边缘 {edgeConnectMode.edge === "left" ? "←" : edgeConnectMode.edge === "right" ? "→" : edgeConnectMode.edge === "top" ? "↑" : "↓"}
            <button
              className="ml-2 underline"
              onClick={cancelEdgeConnect}
              style={{ color: "#fff" }}
            >
              取消
            </button>
            ...
            {monitors.filter((m) => m.enabled).map((mon) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Edge click zones for connection mode */}
                  {isSelected && !isDraggingThis && (
                    <>
                      {/* Top edge zone */}
                      <div
                        className="absolute hover:bg-blue-500/30 transition-colors cursor-crosshair"
                        style={{
                          top: -6, left: "20%", width: "60%", height: 12,
                          zIndex: 25,
                        }}
                        onClick={(e) => { e.stopPropagation(); handleEdgeClick(mon.id, "top"); }}
                        title="点击此边缘连接"
                      />
                      {/* Bottom edge zone */}
                      <div
                        className="absolute hover:bg-blue-500/30 transition-colors cursor-crosshair"
                        style={{
                          bottom: -6, left: "20%", width: "60%", height: 12,
                          zIndex: 25,
                        }}
                        onClick={(e) => { e.stopPropagation(); handleEdgeClick(mon.id, "bottom"); }}
                        title="点击此边缘连接"
                      />
                      {/* Left edge zone */}
                      <div
                        className="absolute hover:bg-blue-500/30 transition-colors cursor-crosshair"
                        style={{
                          left: -6, top: "20%", width: 12, height: "60%",
                          zIndex: 25,
                        }}
                        onClick={(e) => { e.stopPropagation(); handleEdgeClick(mon.id, "left"); }}
                        title="点击此边缘连接"
                      />
                      {/* Right edge zone */}
                      <div
                        className="absolute hover:bg-blue-500/30 transition-colors cursor-crosshair"
                        style={{
                          right: -6, top: "20%", width: 12, height: "60%",
                          zIndex: 25,
                        }}
                        onClick={(e) => { e.stopPropagation(); handleEdgeClick(mon.id, "right"); }}
                        title="点击此边缘连接"
                      />
                    </>
                  )}
