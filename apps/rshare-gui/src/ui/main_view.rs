                ...
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for (index, entry) in activity_log.iter().take(8).enumerate() {
                            ...
                            ui.horizontal(|ui| {
                                ui.colored_label(egui::Color32::from_rgb(100, 170, 255), "●");
                                ui.label(entry);
                            });
                        }
                    });
    ...
    fn show_device_panel(
        ...
        self.panel_frame(egui::Color32::from_rgb(19, 24, 36))
            .show(ui, |ui| {
                ...
                if devices.is_empty() {
                    ui.label(
                        "No devices discovered yet. Keep the service running and wait for peers.",
                    );
                } else {
...
mod tests {
    ...
    fn narrow_width_uses_stacked_dashboard_layout() {
        assert_eq!(
            DashboardContentLayout::for_width(720.0),
            DashboardContentLayout::Stacked
        );
    }
    ...
    fn wide_width_uses_split_dashboard_layout() {
        assert_eq!(
            DashboardContentLayout::for_width(1180.0),
            DashboardContentLayout::Split
        );
    }
