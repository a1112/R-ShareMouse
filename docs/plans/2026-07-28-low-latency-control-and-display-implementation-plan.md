...
`performance-nightly-windows.yml` runs only on `[self-hosted, Windows, X64, rshare-perf]`, prevents overlapping jobs, uses `cargo --locked`/`npm ci`, records power plan/CPU affinity/runner fingerprint, warms 30 seconds, and runs exactly five complete same-config runs. It requires a matching baseline manifest entry from the protected default branch, verifies artifact/config hashes and the merged approval PR as specified in Task 2, and uploads all raw histograms. With CV >10% on comparative latency/rate metrics, it archives the whole unstable batch and reruns the entire five-run batch once; it never drops or selectively replaces a run. A second unstable batch fails. The 100 ms stall recovery is instead checked in every run against its absolute 20 ms correctness bound, because near-zero loopback recovery is dominated by 1 us timer quantization. It restores the original power plan in `finally`; workflow code never auto-updates a baseline to match a regression.
