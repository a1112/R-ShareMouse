...
For keyboard/mouse driver validation, run `scripts\driver\start-hid-validation.ps1 -EnableInputClassFilters` from a normal shell, or run `scripts\driver\validate-hid.ps1 -EnableInputClassFilters` from an elevated PowerShell. The HID flow builds and installs `rshare-filter` plus `rshare-vhid`, enables the keyboard and mouse class `UpperFilters` entry only when `-EnableInputClassFilters` is supplied, checks that the loaded filter driver is new enough, checks `rshare-driver-probe filter stats` for nonzero keyboard and mouse class attachment counters, checks `rshare-driver-probe vhid status`, runs `rshare-driver-probe vhid inject-smoke` for keyboard, mouse movement, mouse button and wheel reports, verifies virtual HID injection increments the filter keyboard and mouse event counters, runs `rshare-driver-probe filter test`, then waits for real keyboard and mouse hardware events with `rshare-driver-probe filter watch-keyboard` and `rshare-driver-probe filter watch-mouse`. Enabling class filters requires a Windows restart or reboot before real hardware capture can be trusted; after reboot, re-run `scripts\driver\start-hid-validation.ps1 -SkipBuild -SkipInstall` from a normal shell, or run `scripts\driver\validate-hid.ps1 -SkipBuild -SkipInstall` from an elevated PowerShell, to collect the capture evidence. To collect that evidence automatically after reboot, run `scripts\driver\register-hid-post-reboot-validation.ps1` from an elevated PowerShell before rebooting; it registers the one-shot `RShareMouse-HidPostRebootValidation` logon task that runs `validate-hid.ps1 -SkipBuild -SkipInstall` and writes `hid-validation-post-reboot-*.log` under `target\driver-validation`. `scripts\driver\restart-input-class-devices.ps1 -ConfirmRestart` can restart the keyboard and mouse device stacks from an elevated shell, and `scripts\driver\start-hid-validation.ps1 -SkipBuild -SkipInstall -RestartInputDeviceStacks` can do the restart before continuing validation when the loaded filter version is already current but attachment counters are missing. It does not unload an older loaded filter driver; if `filter status` still reports an old version, reboot Windows. From the starter script, transcripts are written under `target\driver-validation`.

The filter exposes `IOCTL_RSHARE_WAIT_EVENT` for cancellable, event-driven capture. Production readers and the probe watch commands use this boundary instead of polling `IOCTL_RSHARE_READ_EVENT`; the legacy read IOCTL remains available for compatibility and diagnostics. `IOCTL_RSHARE_QUERY_FILTER_WAIT_STATE` reports the number of requests currently pending on the filter-specific manual wait queue for live validation.
